...
const AllUserInfo = ({ userAllData, setuserAllData }) => {
  ...
  // Badge color based on streak length
  ...
  const handleComplete = async (habit) => {
    try {
      const res = await fetch(
        `https://shourav-havittracker-server.onrender.com/User/${habit._id}/complete`,
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await fetch(
          `https://shourav-havittracker-server.onrender.com/User/${id}`,
          {
            method: "DELETE",
          }
        );
